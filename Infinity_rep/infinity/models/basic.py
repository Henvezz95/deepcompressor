...
class SelfAttention(nn.Module):
    def __init__(
        ...
        batch_size=2, pad_to_multiplier=1, rope2d_normalized_by_hw=0, kv_quant_enabled=False
    ):
        """
    ...
    def forward(self, x, attn_bias_or_two_vector: Union[torch.Tensor, Tuple[torch.IntTensor, torch.IntTensor]], attn_fn=None, scale_schedule=None, rope2d_freqs_grid=None, scale_ind=0):
        ...
        if isinstance(self.mat_qkv, nn.Linear):
            # This path will be taken by the original, unquantized model.
            custom_bias = torch.cat((self.q_bias, self.zero_k_bias, self.v_bias))
            qkv = F.linear(input=x, weight=self.mat_qkv.weight, bias=custom_bias)
        else:
            # This path will be taken after you swap in your custom SVDQuantLinear layer.
            # It correctly calls the forward() method of your layer.
            qkv = self.mat_qkv(x)

        # The reshape operation is the same for both paths
...
class CrossAttnBlock(nn.Module):
    ...
    def print_stats(self, tensor, name):
        """A helper function to print tensor statistics."""
        if tensor is None:
            return
        
        # Get the block name from the 'name' attribute we set in the test script
        block_name = getattr(self, 'name', 'UnknownBlock')
        
        # The .item() calls are the fix for the TypeError
        stats = (
            f"  - {name:<12}: "
        ...
        print(f"[{block_name}] {stats}")

    # NOTE: attn_bias_or_two_vector is None during inference
    def forward(self, x, cond_BD, ca_kv, attn_bias_or_two_vector, attn_fn=None, scale_schedule=None, rope2d_freqs_grid=None, scale_ind=0, **kwargs):    # todo: minGPT and vqgan also uses pre-norm, just like this, while MaskGiT uses post-norm
        ...
        if self.fused_norm_func is None:
            ...
            if self.checkpointing_sa_only and self.training:
                x_sa = checkpoint(self.sa, x_sa, attn_bias_or_two_vector, attn_fn, scale_schedule, rope2d_freqs_grid, use_reentrant=False, **kwargs)
            else:
                x_sa = self.sa(x_sa, attn_bias_or_two_vector, attn_fn, scale_schedule, rope2d_freqs_grid, **kwargs)
            ...
            #self.print_stats(x_sa, "SA Input")
            if self.checkpointing_sa_only and self.training:
                x_sa = checkpoint(self.sa, x_sa, attn_bias_or_two_vector, attn_fn, scale_schedule, rope2d_freqs_grid, use_reentrant=False, **kwargs)
            else:
                x_sa = self.sa(x_sa, attn_bias_or_two_vector, attn_fn, scale_schedule, rope2d_freqs_grid, scale_ind=scale_ind, **kwargs)
            x = x + self.drop_path(x_sa.mul_(gamma1))
            x = x + self.ca(self.ca_norm(x), ca_kv).float().mul_(self.ca_gamma)
